...
function content_save(){
	...
	$p_content=preg_replace_callback('/!\[(.*)\]\s?\((.*)(.webp|.png|.gif|.jpg|.jpeg|.svg)(.*)\)/',function($match){return str_replace(DOC_PATH,"{{DOC_PATH}}",$match[0]);},$p_content);
	// replace url in images
	$p_content=preg_replace_callback('/\[(.*)\]:\s?(.*)(.webp|.png|.gif|.jpg|.jpeg|.svg|)/',function($match){return str_replace(DOC_PATH,"{{DOC_PATH}}",$match[0]);},$p_content);
	// replace path in url
...
function image_upload_ajax(){
	...
	// check extension
	if(!in_array($image['ext'],array("webp", "png","jpg","jpeg","gif","svg"))){
		// error
		echo json_encode(array("error"=>1,"code"=>"扩展名不被允许","file"=>$image));
		// return
		return false;
	}
	// check file type
	if(!in_array($image["type"],array("image/webp", "image/png","image/gif","image/jpg","image/jpeg","image/svg+xml"))){
		// error
		echo json_encode(array("error"=>1,"code"=>"文件不被允许","file"=>$image));
		// return
		return false;
	}
	// make file name
